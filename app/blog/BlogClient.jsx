...
function TagPill({ tag, small }) {
  ...
  return (
    <span style={{
      fontFamily: 'var(--mono)', fontSize: small ? '10px' : '11px', letterSpacing: '0.06em',
  ...
  return (
    <div style={{ minHeight: '100vh' }}>
      ...
      <div style={{ padding: '48px 40px 36px', borderBottom: '1px solid var(--border)' }}>
        <div style={{ fontFamily: 'var(--mono)', fontSize: '12px', color: 'var(--muted-hi)', letterSpacing: '0.1em', marginBottom: '12px', display: 'flex', alignItems: 'center', gap: '10px' }}>
          <span style={{ width: '18px', height: '1px', background: 'var(--muted-hi)', display: 'inline-block' }} />
          write-ups &amp; incident reports
        </div>
        <h1 style={{ fontFamily: 'var(--mono)', fontSize: '36px', fontWeight: '500', color: 'var(--text)', letterSpacing: '-0.02em', marginBottom: '10px' }}>
          ./blog
        </h1>
        <p style={{ fontSize: '16px', color: 'var(--muted-hi)', fontWeight: '300' }}>
      ...
      <div style={{ padding: '20px 40px', borderBottom: '1px solid var(--border)', display: 'flex', gap: '12px', alignItems: 'center', flexWrap: 'wrap' }}>
        <div style={{ position: 'relative', flex: 1, minWidth: '200px' }}>
          ...
          <input type="text" placeholder="search posts..." value={query} onChange={e => setQuery(e.target.value)}
            style={{ width: '100%', background: 'var(--bg2)', border: '1px solid var(--border-mid)', borderRadius: '5px', padding: '9px 12px 9px 36px', fontFamily: 'var(--mono)', fontSize: '12px', color: 'var(--text)', outline: 'none' }} />
        </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 250px' }}>
        <div style={{ borderRight: '1px solid var(--border)' }}>
          ...
          {pinned && (activeTag === 'all' || pinned.tags.includes(activeTag)) && (
            <Link href={`/blog/${pinned.slug}`} style={{ textDecoration: 'none', display: 'block' }}>
              <div style={{ padding: '32px 40px', borderBottom: '1px solid var(--border)', borderLeft: '3px solid var(--green)', background: 'rgba(61,219,114,0.025)', transition: 'background 0.15s' }}
                ...
                <p style={{ fontFamily: 'var(--mono)', fontSize: '10px', letterSpacing: '0.12em', textTransform: 'uppercase', color: 'var(--green)', marginBottom: '10px' }}>
                  priority · pinned
                </p>
                <h2 style={{ fontFamily: 'var(--mono)', fontSize: '22px', fontWeight: '500', color: 'var(--text)', lineHeight: '1.3', marginBottom: '10px', letterSpacing: '-0.01em' }}>
                  {pinned.title}
                </h2>
                <p style={{ fontSize: '15px', color: 'var(--muted-hi)', lineHeight: '1.75', fontWeight: '300', marginBottom: '16px', maxWidth: '560px' }}>
                  {pinned.excerpt}
                </p>
                <div style={{ display: 'flex', alignItems: 'center', gap: '10px', flexWrap: 'wrap' }}>
                  {pinned.tags.map(t => <TagPill key={t} tag={t} />)}
                  <span style={{ fontFamily: 'var(--mono)', fontSize: '11px', color: 'var(--muted)' }}>{pinned.date}</span>
                  <span style={{ fontFamily: 'var(--mono)', fontSize: '11px', color: 'var(--muted)' }}>{pinned.readMin} min read</span>
                </div>
              </div>
            </Link>
          )}
          ...
          {filtered.map(post => (
            <Link key={post.slug} href={`/blog/${post.slug}`} style={{ textDecoration: 'none', display: 'block' }}>
              <div className="blog-row" style={{ padding: '24px 40px', borderBottom: '1px solid var(--border)' }}>
                <div style={{ display: 'flex', gap: '20px', alignItems: 'flex-start' }}>
                  <span style={{ fontFamily: 'var(--mono)', fontSize: '11px', color: 'var(--muted)', minWidth: '56px', paddingTop: '3px', flexShrink: 0 }}>
                    {post.date}
                  </span>
                  <div style={{ flex: 1 }}>
                    <p className="blog-row-title" style={{ fontSize: '16px', fontWeight: '500', lineHeight: '1.4', marginBottom: '6px' }}>
                      {post.title}
                    </p>
                    <p style={{ fontSize: '14px', color: 'var(--muted-hi)', lineHeight: '1.65', fontWeight: '300', marginBottom: '10px' }}>
                      {post.excerpt}
                    </p>
                    <div style={{ display: 'flex', gap: '8px', alignItems: 'center', flexWrap: 'wrap' }}>
                      {post.tags.map(t => <TagPill key={t} tag={t} small />)}
                      <span style={{ fontFamily: 'var(--mono)', fontSize: '10px', color: 'var(--muted)' }}>{post.readMin} min read</span>
                    </div>
                  </div>
                </div>
              </div>
          ...
          {filtered.length === 0 && !pinned && (
            <div style={{ padding: '48px 40px', fontFamily: 'var(--mono)', fontSize: '13px', color: 'var(--muted)' }}>
        ...
        <aside style={{ padding: '28px 24px' }}>
          <div style={{ marginBottom: '28px' }}>
            <div style={{ fontFamily: 'var(--mono)', fontSize: '10px', letterSpacing: '0.12em', textTransform: 'uppercase', color: 'var(--muted)', marginBottom: '14px', display: 'flex', alignItems: 'center', gap: '8px' }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '8px' }}>
              {[
                ...
                <div key={lbl} style={{ background: 'var(--bg2)', borderRadius: '5px', padding: '12px' }}>
                  <div style={{ fontFamily: 'var(--mono)', fontSize: '22px', fontWeight: '500', color: 'var(--text)', lineHeight: 1, marginBottom: '5px' }}>{val}</div>
                  <div style={{ fontFamily: 'var(--mono)', fontSize: '10px', color: 'var(--muted)', letterSpacing: '0.06em', textTransform: 'uppercase' }}>{lbl}</div>
                </div>
              ))}
            </div>
          </div>
          ...
          <div style={{ marginBottom: '28px' }}>
            ...
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: '6px' }}>
              {ALL_TAGS.map(tag => (
                <button key={tag} onClick={() => setActiveTag(tag === activeTag ? 'all' : tag)} style={{
                  fontFamily: 'var(--mono)', fontSize: '11px', padding: '4px 10px', borderRadius: '3px',
          ...
          <div>
            ...
            {posts.slice(0, 4).map((p, i) => (
              <Link key={p.slug} href={`/blog/${p.slug}`} style={{ textDecoration: 'none', display: 'block', paddingBottom: '10px', marginBottom: '10px', borderBottom: i < 3 ? '1px solid var(--border)' : 'none' }}>
                <p style={{ fontSize: '13px', color: 'var(--muted-hi)', lineHeight: '1.4', marginBottom: '3px' }}>{p.title}</p>
                <p style={{ fontFamily: 'var(--mono)', fontSize: '10px', color: 'var(--muted)' }}>{p.date} · {p.readMin} min</p>
              </Link>
            ))}
          </div>
        </aside>
