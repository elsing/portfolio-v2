...
const BADGES = [
  ...
  { label: 'motorbiker',               style: 'moto', icon: true },
];

const BADGE_STYLES = {
  green: { color: 'var(--green)', borderColor: 'rgba(61,219,114,0.28)',  background: 'rgba(61,219,114,0.06)' },
  amber: { color: 'var(--amber)', borderColor: 'rgba(212,168,75,0.25)', background: 'rgba(212,168,75,0.05)' },
  moto:  { color: 'var(--muted-hi)', borderColor: 'var(--border-mid)',  background: 'transparent' },
};
...
function MotoBadge() {
  return (
    <span className="inline-flex items-center gap-1" style={{
      ...
      border: '1px solid var(--border-mid)', color: 'var(--muted-hi)',
    }}>
      <svg width="17" height="11" viewBox="0 0 32 18" fill="none" aria-hidden="true">
...
export default function Hero() {
  return (
    /*
     * padding-right: 5vw nudges the content block rightward so it
     * occupies more of the viewport without a hard pixel cutoff.
     * maxWidth: 900px lets the name sit large without wrapping early.
     */
    <section
      className="relative"
      style={{ zIndex: 2, padding: '68px 5vw 0 40px', maxWidth: '900px' }}
    >
      {/* Eyebrow */}
      <div className="flex items-center gap-2" style={{
        fontFamily: 'var(--mono)', fontSize: '12px', color: 'var(--muted-hi)',
        letterSpacing: '0.1em', marginBottom: '26px',
      }}>
      ...
      {/* Name — 72px */}
      <h1 style={{
        fontFamily: 'var(--mono)', fontSize: '72px', fontWeight: '500',
        color: '#fff', letterSpacing: '-0.025em', lineHeight: '1.05', marginBottom: '26px',
      }}>
        Elliot Singer
      ...
      {/* Badges — slightly larger text */}
      <div className="flex flex-wrap gap-2" style={{ marginBottom: '30px' }}>
        {BADGES.map(({ label, style, icon }) =>
          icon ? <MotoBadge key={label} /> : (
            <span key={label} style={{
              fontFamily: 'var(--mono)', fontSize: '11px', letterSpacing: '0.04em',
              padding: '5px 11px', borderRadius: '4px',
              border: `1px solid ${BADGE_STYLES[style].borderColor}`,
              color: BADGE_STYLES[style].color,
              background: BADGE_STYLES[style].background,
            }}>
              {label}
            </span>
          )
      ...
      {/* Bio */}
      <p style={{
        fontSize: '16px', color: 'var(--muted-hi)', lineHeight: '1.85',
        maxWidth: '580px', marginBottom: '38px', fontWeight: '300',
      }}>
      ...
      {/* CTAs */}
      <div className="flex flex-wrap items-center gap-2" style={{ marginBottom: '80px' }}>
        <Link href="/homelab" style={{
          fontFamily: 'var(--mono)', fontSize: '12px', padding: '10px 22px',
          background: 'var(--green)', color: '#111', border: '1px solid transparent',
          borderRadius: '5px', fontWeight: '500', letterSpacing: '0.04em',
        ...
        {GHOST_LINKS.map(({ href, label, external }) => (
          <Link key={label} href={href}
            ...
            className="btn-ghost"
            style={{
              fontFamily: 'var(--mono)', fontSize: '12px', padding: '9px 20px',
